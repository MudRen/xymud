...
varargs int open_door(string dir, int from_other_side)
{
    ...
    if ( query("open_day") && NATURE_D->query_current_day_phase()<6 )
            return notify_fail("\n城楼上官兵嚷道：天已深，想要开门，要等明早！\n\n");
	exits = query("exits");
...
varargs int close_door(string dir, int from_other_side)
{
   ...
   if ( query("no_close") && NATURE_D->query_current_day_phase()>= 6)  return notify_fail(doors[dir]["name"]+"不是随便便能关闭的。\n");
	exits = query("exits");
	if( !mapp(exits) || undefinedp(exits[dir]) )
