...
string zname(object);
#include <ansi.h>
#define DIRNAME "/d/baihuagu/npc/"

#define MYFILE "/d/baihuagu/npc/zhangmen.c"
...
void init()
{
	...
	me->create_identity("/d/baihuagu/npc/ningmou", "/d/baihuagu/huahai");
...
string zm_apply()
{
	...
	ob->set_temp("baihuagu_zm_applied", 1);
	return "我广寒宫虽不倚强为胜，但作为掌门师姐要多处照顾各位兄弟姐妹，\n不强是不行的。如有意出掌此位，还请一战。\n";
}
...
int accept_fight(object ob)
{
	...
	if (ob->query_temp("baihuagu_zm_applied"))
	{
...
int check_result(object me, object ob)
{
	...
	if (((int)me->query("kee")*100/(1+my_max_kee)) <= 50 )
	{
		...
		if (me->query_temp("zhangmen/kill") && ob->query_temp("baihuagu_zm_applied"))
		{
			message_vision ("$N皱了皱眉道：此次比武我未能静心尽力，希望重新来过。\n", me);
			return 1;
		}

		if( ob->query_temp("baihuagu_zm_applied") != 1 )
			return 1;	
		...
		{
			message_vision ("$N躬身对$n说道：恭请见过凝眸仙子。\n",me,ob);
...
void find_master (object me, object ob)
{
	...
	{
		message_vision ("$N见到凝眸仙子低声说了几句话，又抬起头朝$n使了个眼色。\n\n",me,ob);
...
void master_announce (object me, object who, object ob)
{
	...
	ob->delete_temp("baihuagu_zm_applied");
	me->set("new_player",1);
...
*/
	return "百花谷传人"; //for moon only, others use above.
}
