...
int do_complain(string arg) 
{
	object ob=this_player();
  	...
  	if( me!=present(arg,environment(me)) ) 
  		return 0;
  	message_vision("$N对着$n叫道：“"+RANK_D->query_self_rude(ob)+"早已超出三界之外，不在五行之中，已不归阴司管辖，怎么又来勾我？”\n"NOR,ob,me);
  	EMOTE_D->do_emote(ob, "bug",geteuid(me),CYN,0,0,0);
  	if( !complainee) 
  	{
    		message_vision("\n$N皱了皱眉头，对$n说道：“你我阴阳殊途，"+"我不治你私闯幽冥之罪，也就是了。还不快快退下！”\n\n",me,ob);
    		return 1;
  	}
  	if( name[complainee]!=me->name()) 
  	{
    		message_vision("\n$N皱了皱眉头，对$n说道：“此事非我经手，"+"冤有头，债有主，哪个遣人勾你，你去找哪个吧。”\n\n",me,ob);
    		ob->set_temp("dntg_hell/next",get_next(me));
    		return 1;
  	}
  	ob->delete_temp("dntg_hell/ready");  
  	message_vision("\n$N恼羞成怒道：“本王叫你三更死，谁敢留你到五更。"+"待我亲自送你去见崔判官！”\n",me);
...
void die() 
{
	object me=this_object();
  	...
  	ob->remove_enemy(me);
  	powerup(0,1);
	me->command("say "+ob->name()+RANK_D->query_respect(ob)+"神通广大，小王甘拜下风。");
  	next=get_next(me);
  	ob->set_temp("dntg_hell/next",next);
  	message_vision("$N翻了翻案上的文书，对$n说道：“此事乃"+name[next]+"传文至此，要小王勾拿"+RANK_D->query_respect(ob)+"，"+RANK_D->query_respect(ob)+"还是去找他理论吧。\n\n",me,ob);
...
int do_kill(string arg) 
{
	...
	if( !arg) 
  	...
  	if( me!=present(arg,environment(me))) 
  		return 0;
  	if( ob->query_temp("dntg_hell/ready")!=me) 
  		return 0;
  	if( ob->query("dntg/hell")=="done") 
  		return 0;
  	if( !next) 
  		return 0;
	...
	message_vision("$N吓得魂飞魄散，颤身说：“罢了罢了，依你，都依你。”\n",me);
   	...
   	me->command("chat "+ob->name()+"这"+RANK_D->query_rude(ob)+"逞强行凶，不服拘唤，大闹罗森，强销名号，待我上天禀明玉帝，再来拿你！");
