...
LONG);
	...
	set("item_desc", ([
		"guo" : " 一口大铁锅，往里面加东西用(add)，煎药用(jian yao)。\n", 
          	"pen" : "一只白瓷盆，盆中盛满了清水，要想制冰(make_ice)用它就可以了。\n", 
        ]));
	set("exits", ([
		"west" : __DIR__"shanpo",
	]));
...
int do_add(string arg)
{
	...
	if(item_name=="water")
	{
        ...
        if( item->query("cook/type") == "shui" ) 
        	me->set("cook/allow",1);
        message_vision( "$N将一"+item->query("unit")+item->query("name")+"扔进锅里。\n",who);
        set("item_desc/guo" , " 一口大铁锅(add)，煎药用(jian yao)，里面乱七八糟的不知放了些什么。\n");
...
int do_jian(string arg)
{
        ...
        if(!arg || arg != "yao") 
        	return notify_fail("煎啥？\n");
        if( !me->query("cook/allow") ) 
        	return notify_fail("锅里连水都没有，怎么煎？\n");

        prices = me->query("cook/shui")*me->query("cook/she")*me->query("cook/xie")*me->query("cook/hama")*me->query("cook/zhizhu")*me->query("cook/wugong");
