...
{ 
	...
	if( !target )
        ...
        {
        	message_vision("\n\n"HIR"$N"HIR"后退几步，口内吐出一粒红珠，有碗口大小，望$n"HIR"顶门打去！"NOR"\n",me,target);
        	if( random(3)==0 )
        		message_vision("结果被$N闪开了。\n",target);
        	else
        	{
        		target->start_busy(5);
        		message_vision(HIR"结果$N"HIR"被打了正着，顿时筋松骨酥，口吐鲜血！"NOR"\n",target);
        		damage = target->query_maxkee()/2;
	...
	if( weapon && !me->is_busy() )
	{
		...
		{
			if( random(3)==0 && time()>=(me->query_temp("xueshan_chaofeng")+5) )
				command("perform chaofeng");
			if( random(2)==0 && time()-(int)me->query_temp("feijian_busy")>=4 )
				command("perform feijian");
			if( random(5)==0 )
				command("perform fengwu");
			if( random(6)==0 )
			{
				weapon->unequip();
				command("perform suoxin");
				weapon->wield();
			}
			weapon2 = present("blade",me);
			if( weapon2 && !me->is_busy() )
			{
				weapon->unequip();
				weapon2->wield();
				if( random(4)==0 )
					command("perform bingxue");
				if( random(3)==0 )
					command("perform daobo");
				else if( random(3)==0 )
...
void create()
{
        set_name("戴礼",({"dai li","dai","li"}));
        set_level(110);
        set("nickname", HIC"鸡犬不宁"NOR);
        set("title","天哮大仙");
        set("gender", "男性" );
        set("age", 30);
        set("long", "
...
void dies()
{
	...
	corpse->set_name("土狗尸体",({"tugou's corpse","corpse"}));
	corpse->set("long","一只土狗的尸体。\n");
	if( corpse->move(environment()) )
		message_vision("\n红光散去，但见那遗下的尸骸却是一只土狗，原来是谁的一缕神识栖宿于此肉身作怪而已。\n",this_object());
