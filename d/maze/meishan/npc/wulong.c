...
void create()
{
	set_name("吴龙",({"wu long","wu","long"}));
  	set_level(110);
  	set("title", HIC"千肢万节"NOR);
  	set("title","百足大仙");
  	set("gender", "男性" );
  	set("age", 30);
  	set("long", "
...
void dies()
{
	...
	corpse->set_name("蜈蚣尸体",({"wugong's corpse","corpse"}));
	corpse->set("long","一条蜈蚣的尸体。\n");
	if( corpse->move(environment()) )
		message_vision("\n黄光散去，但见那遗下的尸骸却是一条百尺长的蜈蚣，原来是谁的一缕神识栖宿于此肉身作怪而已。\n",this_object());
