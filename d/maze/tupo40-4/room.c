...
void alternative_die(object who)
{
        ...
        file = sprintf("/data/maze/%c/%s/tupo40-4/enter.c",who->query("id")[0],who->query("id"));
        if( file_size(file)>0 )
        	who->move(file,2);
	else
	{
		tell_object(who,HIG"【副本】星石巨坑：地图已被销毁，你退出了副本地图。\n"NOR);
		who->delete_temp("maze");
		who->move("/d/gao/tulu",2);
	}
