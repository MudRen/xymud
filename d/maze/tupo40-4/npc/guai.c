...
string *Names = ({
	"火蜘蛛","huo zhizhu","焰鬼","yan gui","魔火","mo huo",
});	
...
string long()
{
	string str,long = "天造火极之地「"HIR"星石巨坑"NOR"」之中的怪物。\n";
...
void reward_ob(object who)
{
	...
	{	
        ...
        if( random(succ)>10 )
        {
                file = ("/d/maze/tupo40-4/obj/xiu");
                ob = new(file);
                ob->set("owner_id",who->query("id"));
                if( !ob->move(who) )
                {
                        destruct(ob);
                        tell_object(who,HIR"【副本】星石巨坑：你的背包满了。\n"NOR);
                }
                else    
                {
                        tell_object(who,HIY"【副本】星石巨坑：你得到了 "HIG+obj_name(ob)+HIY" 。\n"NOR);
                        arg+= obj_name(ob);
                }
        }
        if( random(succ)>10 )
        {
        	...
        	while(i--)
        	{
        		if( random(2) )
	        	{
        	        	ob = new("/d/maze/tupo40-4/obj/armor_sj");
                		if( strsrch(arg,"厚土石")==-1 )
                			arg+= "、厚土石";
                	}
                	else
                	{
                		ob = new("/d/maze/tupo40-4/obj/weapon_sj");
				if( strsrch(arg,"金芒石")==-1 )
        	        ...
        	        else    tell_object(who,HIY"【副本】星石巨坑：你得到了 "HIG+obj_name(ob)+HIY" 。\n"NOR);
               } 
        }
        ...
        {
        	ob = new("/d/maze/tupo40-4/obj/"+dir[random(sizeof(dir))]);
        	if( !ob->move(who) )
		{
        		destruct(ob);
                	tell_object(who,HIR"【副本】星石巨坑：你的背包满了。\n"NOR);
        ...
        if( strlen(arg)>6 && (random(query("my_level"))>3||query("my_level")>6) )       
        	CHANNEL_D->do_channel(this_object(),"rumor","听说"+who->name()+HIM"在高老庄南方的"HIR"星石巨坑"HIM"得到了"+arg+"。"NOR);
        destruct(this_object());        
        return;
}
