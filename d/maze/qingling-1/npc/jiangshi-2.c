...
void create()
{
        set_name("赤僵尸",({"jiang shi","shi","jiangshi","zambie"}));
        set("gender","男性");
...
void reward_ob(object who)
{
        ...
        if( random(succ)>18 )
        {
                file = ("/d/maze/qingling-1/obj/change1");
                ob = new(file);
        ...
        dir = ({
                "armor","cloth","kui","pifeng","shield","shoes",
        });
        ...
        while(i--)
        {
                ...
                if( file )
                {
                        ...
                        ob->set("from","【始皇陵一层副本】");
                        if( this_object()->query_level()<20 )
                                set("env/test",({"HIB","HIC","HIY","HIC","HIY"})[random(5)]); 
                        else    if( random(succ)>25 )
                                set("env/test",({"HIB","HIR"})[random(2)]);
                        else    set("env/test",({"HIB","HIC","HIY"})[random(3)]);
        ...
        if( random(this_object()->query_level())>5 )
        {
