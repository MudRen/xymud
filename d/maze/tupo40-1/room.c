...
{
        set("short","销金窝副本");
        set("channel_id","销金窝副本");
        set("alternative_die",1);
        set("item_desc",([
                "west" : "突然从远处传来一阵好像万剑交击，金龙腾挪、无数铃铛一起震动地金属震鸣声，听在耳朵里让人不寒而栗。\n",
                "east" : "突然从远处传来一阵好像万剑交击，金龙腾挪、无数铃铛一起震动地金属震鸣声，听在耳朵里让人不寒而栗。\n",
                "north" : "突然从远处传来一阵好像万剑交击，金龙腾挪、无数铃铛一起震动地金属震鸣声，听在耳朵里让人不寒而栗。\n",
                "south" : "突然从远处传来一阵好像万剑交击，金龙腾挪、无数铃铛一起震动地金属震鸣声，听在耳朵里让人不寒而栗。\n",
        ])); 
        setup();
}
...
void alternative_die(object who)
{
        ...
        file = sprintf("/data/maze/%c/%s/tupo40-1/enter.c",who->query("id")[0],who->query("id"));
        if( file_size(file)>0 )
        	who->move(file,2);
	else
	{
		tell_object(who,HIG"【副本】销金窝：地图已被销毁，你退出了副本地图。\n"NOR);
		who->delete_temp("maze");
		who->move("/d/shushan/shanjiao",2);
	}
