...
void alternative_die(object who)
{
        ...
        message_vision(NOR"\n( $N"+msg+NOR")\n",who);
        message_vision("\n"HIC"$N"HIC"伤势过重，幸亏有师门的保命符方才性命无忧。\n"NOR,who);  
        file = sprintf("/data/maze/%c/%s/tupo40-5/enter.c",who->query("id")[0],who->query("id"));
        if( file_size(file)>0 )
        	who->move(file,2);
	else
	{
		tell_object(who,HIG"【副本】终南幽谷：地图已被销毁，你退出了副本地图。\n"NOR);
		who->delete_temp("maze");
		who->move("/d/changan/zhongnan",2);
	}
