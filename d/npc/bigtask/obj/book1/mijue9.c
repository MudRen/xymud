...
void create()
{
        set_name(YEL "《盘丝秘诀》" NOR, ({"youming mijue", "mijue"}));
        set_weight(5);
        if (clonep())
		...
		set("no_drop",1);
		set("no_get",1);
                set("long", "一本记录花果山盘丝洞武功和咒术的经书，上面氤氲袅绕。。。\n");
                set("value", 10000);
        }
...
int do_yandu(string arg)
{
        ...
        message_vision(HIB "$N迫不及待地翻阅着《盘丝秘诀》！ \n" NOR, me);  
	me->set("daemon/mijue_pansi","get");
 ...
 this_object()));
        ...
        tell_object (me,"你仿佛对花果山盘丝洞武功和咒术的领悟进入了一个新的境界。\n");
