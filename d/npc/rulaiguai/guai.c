...
{        
	...
	remove_call_out("destruct_ob");
                    call_out("destruct_ob", 3600, this_object());
}
		...
		if (random(3) < 1)  command ("say 我还没准备好，你就开打了，不算数。");
		                   ...
		                   { command ("say 我现在方知天外有天，人外有人，我还要回山多多修炼，告辞了。");
		   	             ...
		   	             ob->delete("rulaitask/get");
                                     if(ob->query("eff_kee") < 1) ob->set("eff_kee",1);
		   	             destruct(me);
		   	             return 1;
		   	            }
...
int do_persuade(string arg)
{
    ...
    if ( !living(ob) )   {write("很显然，"+ob->name()+"听不见你说的话。\n");return 1;}
...
void die()
{       object me = owner();
if ( !me ) { destruct(this_object());return;}
