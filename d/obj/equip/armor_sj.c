...
void create()
{
	...
	set("base_unit", "颗");
	set("unit","些");
	set("long", "玄武泪珠所化，能将可召唤的%d级以下的防具升级(shengji)。\n");
	set("value", 0);
    set("no_put",1);
    //set("no_give",1);
    set("no_get",1);
...
int do_change(string arg)
{
	...
	if( !armor->is_zhaohuan() )
		return err_msg(armor_short(armor)+"不是可召唤的防具，不能升级。\n");
	if( armor->query("equipped") )
		return err_msg("你要先卸除"+armor_short(armor)+"。\n");		
	if( !armor->query("armor_type")
	 || !armor->query("armor_level") )
		return err_msg(armor_short(armor)+"不是防具。\n");
	if( armor->query("armor_level")>query("level") )
		return err_msg(armor_short(armor)+"品质太高，你需要"+query_title(armor->query("armor_level"))+"以上品质的玄武石(玄)。\n");
	my_data = armor->query("armor_prop");
	tmp = keys(my_data);
	if( !tmp || !arrayp(tmp) )
		return err_msg(armor_short(armor)+"属性单一，没升级的必要。\n");
	prop = ([
		"attack"		: "命中率",
        "strength"      : "后天体格",
       	"courage"       : "后天胆识",
	...
	j = sizeof(tmp);
	j = random(j);
	write(HIG"【防具升级】"+armor_short(armor)+HIG"升级成功！\n"NOR);
	for(i=0;i<j;i++)
	{
		type = tmp[i];
		if( !stringp(type) || type=="armor" )
			continue;
		if( my_data[type]>100 )
			continue;			
		k = random(2)+random(armor->query("armor_level")/50);
		...
		if( k>0 )
		{
			...
			{
				if( member_array(type,precent)!=-1 )
					write(HIG"【防具升级】"+armor_short(armor)+HIG"的 "HIR+prop[type]+HIG" 属性增加了"HIR+k+"% "HIG"。\n"NOR);
				else
					write(HIG"【防具升级】"+armor_short(armor)+HIG"的 "HIR+prop[type]+HIG" 属性增加了"HIR+k+" "HIG"点。\n"NOR);	
			}
			else if( file_size(SKILL_D(type+".c"))!=-1 )
				write(HIG"【防具升级】"+armor_short(armor)+HIG"的 "HIR+to_chinese(type)+"有效等级"+HIG" 属性增加了"HIR+k+HIG" 点。\n"NOR);
			else if( sscanf(type,"ad_%s",type) )
			{
				if( file_size(SKILL_D(type+".c"))!=-1 )
					write(HIG"【防具升级】"+armor_short(armor)+HIG"的 "HIR+to_chinese(type)+"技能等级"+HIG" 属性增加了"HIR+k+HIG" 点。\n"NOR);
				else
					write(HIG"【防具升级】"+armor_short(armor)+HIG"的 "HIR+type+HIG" 属性增加了"HIR+k+HIG" 点。\n"NOR);	
			}
			else	write(HIG"【防具升级】"+armor_short(armor)+HIG"的 "HIR+type+HIG" 属性增加了"HIR+k+HIG" 点。\n"NOR);	
		}	
	}
	armor->add("armor_level",1);	
	k = random(2)+random(armor->query("armor_level"))/5;
	k = k/2;
	if(k<1)
		k = 1;
	if( k>0 )
	{
		armor->add("armor_prop/armor",k);
		write(HIG"【防具升级】"+armor_short(armor)+HIG"的防御增加了 "HIR+k+" 点"HIG"。\n"NOR);
	}
	armor->save();
	if( query_amount()>1 )
		add_amount(-1);
