...
void melt()
{
	...
	if( env->is_character() )
	{//a player or a NPC.
                if( (int)env->query_skill("force", 1) < 100 )
		{
			message_vision("$N觉得身上凉凉的，湿湿的，原来是冰刀化了。\n", env);
			env->apply_condition("double_ice_poison", 15);
			//note, 中毒了。
...
"$N运力一震，只见冰花四溅！一把冰刀顿时化为无数碎片向$n激射而去！\n" NOR;
	...
	if(success == 1 )
	{
		...
		//here also apply poison..
              target->apply_condition("double_ice_poison",  5 + (int)me->query("force_factor")/2 + (int)target->query_condition("double_ice_poison") );
