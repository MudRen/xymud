...
void create()
{
        set_name(HIY"《金乌刀法》"NOR, ({"jinwu daofa","jinwu","daofa", "book"}));
        set_weight(1000);
        if( clonep() )
...
int do_read(string arg)
{
	...
	me=this_player();
    	...
    	if (me->query("mark/jinlong")!="jinwu")
    	                return notify_fail("你哪儿偷来的书？\n"); 
    	if( me->is_busy() )
        		return notify_fail("你现在忙着呢，哪有可能静心研读...\n");
   	if( me->is_fighting() )
        		return notify_fail("现在才想到精研金乌刀法，太晚了吧？\n");
	if( (int)me->query_skill("literate", 1) < 50 ) 
		return notify_fail("你看了半天，里面不少字都是它认得你你不认得它。\n");
	if( (int)me->query_skill("jinwu-blade",1)>60 )
		return notify_fail("你在金乌刀法方面已经很有造诣，这本书不会让你长进多少。\n");
	...
	me->improve_skill("jinwu-blade", gain);

	message_vision("$N研究金乌刀法秘籍，似乎有所领悟。\n", me);
