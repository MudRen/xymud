...
void create()
{
        set_name(YEL"《神兵图鉴》"NOR"上册"NOR, ({"shenbing tujian","shenbing","tujian","book"}));
        set_weight(1000);
        if( clonep() )
...
int do_read(string arg)
{
        ...
        if( (int)me->query_skill("parry", 1) < 200 ) 
                return notify_fail("你在拆招方面的造诣太低，实在无法看懂。\n");
        if( (int)me->query_skill("parry",1)>450 )
                return notify_fail("你在拆招方面已经很有造诣，这本书不会让你长进多少。\n");
        ...
        message_vision("$N仔细研读神兵图鉴，似乎对拆招的技巧有所领悟。\n", me);
