...
// skill level will be set to the maximum level of the player.
        set("eff_sen",max_sen);
        set("eff_kee",max_kee);
        set("sen",max_sen);
        set("kee",max_kee);
...
{       
        ...
        if(exp<30000) {
            ...
            exp_reward=180+exp/1900;
            pot_reward=40+exp/19000;
        } else if(exp<3000000) {
            exp_reward=230+exp/19000;
            pot_reward=50+exp/190000;
        } else {
            exp_reward=180+random(200);
            pot_reward=20+random(100);
        }

        pot_reward=pot_reward*(lvl+1);
        exp_reward=exp_reward*(lvl+1);
        if(pot_reward>600) pot_reward=600;
        if(exp_reward>1000) exp_reward=1000;
        me->set("stay_time", time()+600);
        who->set("secmieyao_time",time());
...
void check_room()
{
        ...
        if(time()>query("stay_time")&&!is_fighting())
                leave();
...
/*
                if (random(10)>=5)      
                    {
                    carry_object("/d/obj/suburb/bag");
                    command("drop bag");
                    }     
*/
                    exp_r=610+random(200);
                    pot_r=200+random(50);
////////////////////////////////////
TASK_D->job_reward_player(owner_ob,"daoxing",exp_r,"门派任务");
TASK_D->job_reward_player(owner_ob,"combat_exp",(exp_r/5+1),"门派任务");
TASK_D->job_reward_player(owner_ob,"potential",pot_r,"门派任务");
...
*/
          ...
          tell_object(owner_ob, HIR"\n你得到了"NOR+FLA+COMBAT_D->chinese_daoxing(exp_r)
             +NOR+HIY"道行和"NOR+FLA+chinese_number(exp_r/5)+NOR+ HIY"点实战经验和"NOR+FLA+chinese_number(pot_r)+NOR+ HIY"点潜能！\n"NOR);
        str = HIG""+owner_ob->name()+"为师门杀敌得到"+COMBAT_D->chinese_daoxing(exp_r)+"道行和"+chinese_number(exp_r/5)+"点实战经验和"+chinese_number(pot_r)+"点潜能！"NOR;
//      CHANNEL_D->do_channel(me, "job",str);       
...
{
if( ::heal_up() )  return 1;
else if( !query_heart_beat(this_object()) ) {
