...
int do_enter(string arg)
{
	...
	if ((arg=="jiaozi" || arg=="huajiao" || arg=="jiao")
		...
		set_follow(find_player(query("groom")));
...
int accept_money(object who,int v)
{
	...
	if (!who->query_temp("marrying") || !present(who->query_temp("bride"), environment(who))) {
	...
	if (v==10000)	{
		...
		worker1->set_follow(this_object());
	...
	if (v==20000) {
		...
		worker1->set_follow(this_object());
		worker1->set_temp("no_return",1);
		worker2->set_follow(worker1);
		worker2->set_temp("no_return",1);
		worker3->set_follow(worker1);
