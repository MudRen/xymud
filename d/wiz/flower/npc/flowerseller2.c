...
{ 
        object ob;
        if (!this_player()->query("giftmark/2004-6-1"))
        {
                ...
                ob->move(this_player()); 
                message_vision("\n$N递给$n一个可爱的泡泡。\n",  this_object(),this_player());
message("chat",HIC+"〖"+HIM+"节日快乐"+HIC"〗"+HIG"礼物使者：祝"HIM+this_player()->query("name")
+HIR"童心常葆，六一快乐！！\n"NOR+
HIY"\n\n     │  ╱
         ╮
      "HIR"～ ～ "HIY"──"HIC"   ╬╮
...
{
        object ob;
        if (!this_player()->query("giftmark/2005-1-1"))
        {
                command("say 恭喜恭喜，新年快乐！\n");
                ob=new("d/wiz/obj/lipin2");
                ob->set("onwerid",this_player()->query("id")); 
                ob->move(this_player()); 
                message_vision("\n$N递给$n一个精美的大礼包。\n",  this_object(),this_player());
message("chat",HIC+"〖"+HIM+"新年快乐"+HIC"〗"+HIG"礼物使者：祝"HIM+this_player()->query("name")
+HIR"鸡年吉祥，大吉大利！！\n"NOR,users());
                return 1;
        }
        message_vision("\n$N对$n说道：你拿过礼包了吧？不要太贪心哦！\n",  this_object(),this_player());
