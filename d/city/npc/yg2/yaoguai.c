...
void create()
{
	...
	set("no_cast",1);
	set("env/test",({"","",HIY,HIY,HIY,HIY,HIB,})[random(7)]);
	setup();
}
...
int copy_status(object me,object ob, int lv)
{
	...
	setup();
	add_temp("apply/max_kee",10000+level*5);
	add_temp("apply/max_sen",10000+level*5);
	add_temp("apply/max_force",5000+level*3);
	add_temp("apply/max_mana",5000+level*3);
	add_temp("apply/combat_damage",500+level);
	add_temp("apply/spells_damage",500+level);
	add_temp("apply/combat_def",500+level);
	add_temp("apply/spells_def",500+level);
	powerup(0,1);
	set("force_factor",me->query_maxforce()/25+1);
	set("mana_factor",me->query_maxmana()/25+1);
	return 1;
}
...
void die()
{
	...
	if( owner && stringp(owner) && objectp(owner_ob = find_player(owner)) ) 
	{
			...
			tell_object(owner_ob,HIY+owner_ob->finish_job(job_type(),({exp_r,dx_r,pot_r}))+NOR);
                    	owner_ob->money_add(4000);
                    	tell_object(owner_ob,"你得到了四十两白银的奖励。\n");
                    	lvl = owner_ob->query_job_level("袁天罡任务");
			tell_object(owner_ob,HIY"【噩梦】本轮已经完成 "HIR+lvl+"/10"HIY"个，已完成"HIR+owner_ob->query_temp("mark/evil_yuan")+HIY"轮。\n"NOR);                    		
                    	if( lvl>=10 )
                    	{
                    		tell_object(owner_ob,HIY"恭喜！你完成了一轮噩梦级别的官府任务。\n"NOR);
                    		owner_ob->add_temp("mark/evil_yuan",1);
                    		owner_ob->money_add(10000);
                    		tell_object(owner_ob,"你得到了一两黄金的追加奖励。\n");
                    	}
		}
	}
        destruct(this_object());
