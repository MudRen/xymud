...
void die()
{
        ...
        string str;
	object silver;
        int max_kee, max_sen, ratio, exp_r, pot_r,dx_r,lvl;
	...
	message_vision("\n"HIB"$N"HIB"惨叫一声，吐了几口"NOR RED"鲜血"NOR HIB"，化作一道青烟散去。\n"NOR,this_object());
        owner = query("owner");
	if( owner && stringp(owner) && objectp(owner_ob = find_player(owner)) ) 
	{
        	if(environment(owner_ob)==environment())
        	{
        		...
        		if(ratio<0) ratio=0;
                	exp_r = exp_reward;
        		pot_r = pot_reward*2/3;
			dx_r = exp_reward/15+random(exp_reward/15)+1;
			...
			tell_object(owner_ob,HIY"【噩梦】本轮已经完成 "HIR+lvl+"/10"HIY"个，已完成"HIR+owner_ob->query_temp("mark/evil_yuan")+HIY"轮。\n"NOR);                    		
                    	if( lvl==10 )
                    	{
                    		...
                    		owner_ob->money_add(20000);
                    		tell_object(owner_ob,"你得到了二两黄金的追加奖励。\n");
                    	}
		}
	}
        destruct(this_object());
