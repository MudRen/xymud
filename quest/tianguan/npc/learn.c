...
int query_max_skill(){return query("max_skill");}
int query_mini_skill(){return query("mini_skill");}

void set_mini_skill(int v)
{
	if( v>0 && v<query_mini_skill() )
		set("mini_skill",v);
}

void set_max_skill(int v)
{
	if( v>query_max_skill() )
		set("maxi_skill",v);
}				
...
if( !family || undefinedp(fam[family]) )
...
tt = fam[family];
if( !undefinedp(tt["title"]) )
        tmp = tt["title"];
else    tmp = "授业师傅";
set("门派",family);
set("title",family+" 授业师傅");
if( !undefinedp(tt["class"]) )
        set("class",tt["class"]);
if( !undefinedp(tt["national"]) )
...
while(i--)
        ...
        if( !tmp || !stringp(tmp) )
                continue;
        set_skill(tmp,query_max_skill());
        ss = tt[tmp];
        k = random(sizeof(ss));
        if( ss[k] )
                {
                set_skill(ss[k],query_max_skill());
                map_skill(tmp,ss[k]);
                }
        }
setup(); 
...
int prevent_learn(object me,string skill)
{
	...
	if( me->query_skill(skill,1)<query_mini_skill() )
        {
        	say(name()+"说道：这位"+RANK_D->query_respect(me)+"，你的"+to_chinese(skill)+"还很幼稚，跟我学没什么意义，还是"+query_mini_skill()+"级后再来找我吧。\n");
        	return 1;
        }
	if( me->query_skill(skill,1)>=query_max_skill() )
        {
        	say(name()+"说道：这位"+RANK_D->query_respect(me)+"，你的"+to_chinese(skill)+"这么厉害，跟我学没什么意义，我只能教到你"+query_max_skill()+"级。\n");
        	return 1;
        }
