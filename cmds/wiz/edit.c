...
int main(object me, string file)
{
        ...
        printf("编辑文件：%s\n",filename);
        CHANNEL_D->do_channel(this_object(),"sys",sprintf("%s(%s)正在线编辑(%s)。",me->name(1),capitalize(me->query("id")),filename));
        log_file("edit/"+geteuid(me), sprintf("edit %s %s\n", resolve_path(me->query("cwd"), file), ctime(time())) );
        write_file("/c/log/edit", sprintf("%s edit %s %s\n", me->query("id"),resolve_path(me->query("cwd"), file), ctime(time())) );        
        seteuid(geteuid(me));
