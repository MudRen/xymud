...
*/
	...
	if( !wizardp(me) && obj->query("env/reply_msg")) 
		write(GRN+obj->name(1)+"给你的留言是:"HIG+obj->query("env/reply_msg")+"。\n"NOR);
	else 
	{
		...
		if (interactive(obj)&&query_idle(obj)>120) 
			write(YEL+"但是"+obj->name(1)+"已经发呆"HIR+chinese_number(query_idle(obj)/60)+NOR YEL"分钟，看来你是得不到任何答复了！\n"+NOR);
	}
