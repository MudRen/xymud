...
int main(object me, string arg, int silent)
{
	...
	if( dest && objectp(dest))
		...
		return notify_fail("你要去的区域还没有连通。错误地图文件："+file_name(environment(me))+" \n");
	...
	else
	{
		...
		str1="往" + dir + obj->query("short")+"离开。\n";
	}
	if( objectp(thing = me->query_temp("armor/armor")))
		thing_msg = "身披" + thing->query("name");
	else if( objectp(thing = me->query_temp("armor/cloth")))
		thing_msg = "身着" + thing->query("name");
	else if( objectp(thing = me->query_temp("armor/head")))
		thing_msg = "头戴" + thing->query("name");
	else if( objectp(thing = me->query_temp("armor/surcoat"))
	      || objectp(thing = me->query_temp("armor/shield")) )
		thing_msg = "背披" + thing->query("name");
	else if( objectp(thing = me->query_temp("armor/boots")))
		thing_msg = "脚踏" + thing->query("name");
	else if( objectp(thing = me->query_temp("armor/waist")))
		thing_msg = "腰缠" + thing->query("name");
	else if( objectp(thing = me->query_temp("armor/finger"))
	      || objectp(thing = me->query_temp("armor/hands"))
	      || objectp(thing = me->query_temp("armor/wrists"))
	      || objectp(thing = me->query_temp("armor/neck")) )
		thing_msg = "身佩" + thing->query("name");
