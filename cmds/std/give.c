...
varargs int do_give_money(object me,object who,string item,int amount)
{
	if( !me || !who || !item )
		return 0;
      if( (me->query("age")<16 || who->query("age")<16) && userp(me) && userp(who) )
                return notify_fail("未成年人之间禁止金钱交易。\n");
     if( me->query_level()<25 && userp(who) && userp(me) )
           return notify_fail("人物等级25级以下禁止金钱交易。\n");
     if( who->query_level()<25  && userp(who) && userp(me) )
           return notify_fail("人物等级25级以下禁止金钱交易。\n");
	if( amount<1 )
...
message("channel:chat",HIG"【给予精灵】某人："+"据说"+me->query("name")+"给了玩家 "HIW+who->query("name")+HIG" 一"+obj->query("unit")+obj->name()+"。\n"NOR,users());
        ...
        if(!userp(who) && !userp(obj) && !obj->give_npc() ) destruct(obj); // mon 4/25/98
me->start_busy(1);
if( userp(me) ) me->save();
