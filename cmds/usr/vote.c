...
if( !arg || sscanf(arg, "%s %s %s", choice, chan, playerid)!=3 ) 
        return notify_fail(SYNTAX);
if (me->query("age") < age_limit) 
        return notify_fail("你年龄不满"+chinese_number(age_limit)+"，没有投票权。\n");
if( me->query("gin")<5 )
	return notify_fail("你活力不足。\n");
    // sytax errors.
...
if (!me->visible(player) || wizardp(player) ) 
        return notify_fail("目前找不到 "+playerid+" 这个人。\n");
...
if(undefinedp(votes_casted[myvariable]))
        ...
        call_out("nullify_vote", nullify_vote_time, myvariable, choicename, playername);
        }
diff=total_votes_required-votes_count;
// announce to mu
if (diff>0)
        message("vote", HIW"【表决】"+myname+"投票"+choicename+playername+"的闲聊频道，需要" + chinese_number(total_votes_required)+ "票通过，还差" + chinese_number(diff) +"票！\n"NOR, users);
else    {
        ...
        message("vote", HIW"【表决】"+myname+"投票"+choicename+playername+"的闲聊频道，"+playername+"的交谈频道被"+choicename+"了！\n"NOR, users);
        }
	me->add("gin",-5);        
	return 1;
}
...
total_required=total_required/30;
...
if (!undefinedp(voted_players[myvariable])) 
        {
        message("vote", HIW"【表决】因为没有足够的人响应"+choice+username+"的交谈频道，投票取消了！\n"NOR, users);
        map_delete(voted_players, myvariable);
        map_delete(votes_casted, myvariable);
        }
}
