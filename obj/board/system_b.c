...
int check_news(object me)
{
 	...
 	if(userp(me))
	{
		...
		if(i>10) write("你可以 Help news 来查看大唐西游新闻系统的使用方法。\n"); // 提醒不知道有NEWS系统的玩家使用news
	}
}
...
int main(object me, string arg)
{
	...
	if( !pointerp(notes) || !sizeof(notes) )
		return notify_fail("「大唐西游」目前没有任何新闻。\n");
		
	if( !arg ) 
	{
		if(i>10) write("你可以 Help news 来查看大唐西游新闻系统的使用方法。\n"); 
			return notify_fail(
				"「大唐西游」目前共有"+HIW+chinese_number(sizeof(notes))+NOR"条新闻"+((i==0)? "。":"，您有"+HIY+chinese_number(i)+NOR"条新闻尚未阅读。")+"\n"); }
	...
	"-------------------------------------------------------------------------------\n",
		BBLU HIG"「"HIC"大唐西游"HIG"」第"+chinese_number(num + 1)+"条新闻"NOR, 
