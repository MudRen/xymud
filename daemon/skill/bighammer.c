        ...
        &&  ob = victim->query_temp("weapon") ) 
        {
        	...
        	{	
                	...
                	if( wap > 2 * wdp ) 
                	{
                        	...
                        	message_vision("$N"HIW"只觉得手中" + ob->name() + HIW"一震，险些脱手！\n",victim);
                	else if( wap > wdp / 2 ) 
                	{
                        	message_vision(HIW "只听见「啪」地一声，$N"HIW"手中的" + ob->name()+ HIW"已经断为两截！\n" NOR, victim );
                        	ob->unequip();
                        	ob->move(environment(victim));
				...
				ob->set("weapon_prop", 0);
                	...
                	else	message_vision("$N"HIW"的" + weapon->name() + HIW"和$n"HIW"的" + ob->name()+ HIW"相击，冒出点点的火星。\n", me, victim);
		}
		else	message_vision("$N"HIW"的" + weapon->name() + HIW"和$n"HIW"的" + ob->name()+ HIW"相击，冒出点点的火星。\n", me, victim);
        }
}
