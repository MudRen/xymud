...
int perform(object me, object target)
{
	...
	if( !target
 	 ...
 	 || !me->is_fighting(target) )
		return notify_fail("你要对谁施展这一招「鬼·神·人」？\n");
	...
	message_vision(HIC"$N"HIC"双手划一大圆，猛然托天而举，伴着天上雷鸣电闪，祭出了「鬼·神·人」三招。\n"NOR,me,target);
	...
	//出剑
	if( me->query_skill("zhuihun-sword",1)>350
	 && me->query_skill("sword")>350 
	 && me->query_skill_mapped("sword")=="zhuihun-sword" )
	{
		message_combatd(HIR"$N"HIR"眸中红芒一闪，"+weapon->name()+HIR"上剑气冲天，直扑$n"HIR"。\n"NOR,me,target);
		COMBAT_D->do_attack(me,target,me->query_temp("weapon"),TYPE_PERFORM,0,0,0,me->query_skill("zhuihun-sword",1)/4);
	}
	me->start_busy(2);
	return 1;
}
...
str = @LONG
        外功名称：鬼·神·人
        外功所属：追魂剑
        外功效果：分别以掌、棒、鞭招式对敌连续攻击3次
        	  如果各招式激发为地府技能，则每次均带固定伤害
        	  如追魂剑达到350级并激发，则追加一次带固定伤害的攻击
                  使后自身busy 2 秒
        外功条件：
        	  人物等级10级	
                  追魂剑100级
